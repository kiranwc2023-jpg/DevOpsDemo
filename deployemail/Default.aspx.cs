...
public partial class _Default : System.Web.UI.Page 
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        oplbl.Text = "";
        Panel1.Visible = true;
        using (SqlConnection objConnection = new SqlConnection(@"Server =qltlbbrscmdb01; Database = scmworkbench; Trusted_Connection = Yes;"))
        {
            string env = DropDownList1.SelectedValue.ToString();
            if (env == "US-Staging")
                env = "Staging";
            if (env == "US-Production")
                env = "Production";
            ...
            while (dr.Read())
            {
                ...
                if (status.Contains("Starting"))
                {
                    ...
                    sbjlbl.Text = " BrassRing "  + rntxt.Text + " to " + DropDownList1.SelectedValue.ToString() + " - Starting Deployment";
                    blbl.Text = "The latest version of BrassRing (" + rntxt.Text + ") is being deployed now to " + DropDownList1.SelectedValue.ToString() +
                                 "\r\n" +
                                 "\r\n" + "Hosting Team," +
                                 "\r\n" +
                                 "\r\n" + "Please disable the WUG server alerts for " + DropDownList1.SelectedValue.ToString() + " for next 8 hours. " +
                                 "\r\n" +
                                 "\r\n" + "Enable server alerts upon receiving completion email from us.";
                    
                }
                if (status.Contains("Finished"))
                {

                    sbjlbl.Text = " BrassRing " + rntxt.Text + " to " + DropDownList1.SelectedValue.ToString() + " - Completed Deployment";
                    blbl.Text = "The latest version of BrassRing (" + rntxt.Text + ") has been deployed to " + DropDownList1.SelectedValue.ToString(); //+ " is complete."; //+ "</br>" + "</br>" + "Thanks," + "</br>" + "Kiran";
                }
            }
